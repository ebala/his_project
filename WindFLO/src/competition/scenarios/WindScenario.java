...
public class WindScenario {
	...
	public WindScenario(InputStream xmlFileName) throws Exception {
//		File fXmlFile = new File(xmlFileName);
		...
		wakeFreeEnergy = Double
				.parseDouble(((Node) (doc.getElementsByTagName("WakeFreeEnergy").item(0))).getTextContent());

		// these are currently all the same across scenarios, so aren't in the
		// xml files
		CT = 0.8;
		PRated = 1500.0;
		R = 38.5;
	...
	public double getWblcdfVints(double c, int vintIndex, int ksIndex) {
		return wblcdfValues[(int) ((c - cMin) / wblcdfAccuracy) * (vints.length + 1) * ks.length + vintIndex * ks.length
				+ ksIndex];
	}

	public double getWblcdfVrated(double c, int ksIndex) {
		return wblcdfValues[(int) ((c - cMin) / wblcdfAccuracy) * (vints.length + 1) * ks.length
				+ vints.length * ks.length + ksIndex];

	}

	/**
	 * Returns the filename of the scenario with or without obstacles.
	 * @param number The scenario number (currently, 0 to 9)
	 * @param hasObstacles True in case the scenario with obstacles should be used
	 * @return The filename as {@link String} to the scenario XML file
	 * @throws IOException Exception is thrown in case the file was not found.
	 */
	public static InputStream getScenarioFilename(int number, boolean hasObstacles) throws IOException {
		URL fname = null;
		String file="";
		if (hasObstacles) {
		...
		if (fname.getFile().isEmpty()) {
			...
			throw new IOException("Scenario not found: " + fname);
		}
		
		InputStream is = WindScenario.class.getResourceAsStream(file);
		if (null != is) {
		    return is;
		}
	...
	public static double wblcdf(double x, double sc, double sh) {
